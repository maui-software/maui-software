...
def _convert_if_list_string(value):
    """
    Helper function to convert a space-separated or comma-separated string of floats or integers into a list of floats or integers.
    
    Parameters:
    value (any): The value to check and possibly convert.

    Returns:
    any: The original value or a converted list of floats or integers if the string represents a list of numbers.
    """
...
def _extract_indices_worker(
    ...
    """
    Helper function to extract acoustic indices from audio files and save them temporarily in a CSV file.
    ...
    **kwargs : dict, optional
    ...
    for chunk_index, (index, row) in enumerate(df.iterrows()):
        s, fs = sound.load(row[file_path_col])
        
        if len(s) == 0:
            print(f"Sound loading failed or the file {row['file_path']} is corrupted. Acoustic indices not calculated.")
        else:
            ...
            for method in acoustic_indices_methods:
                ...
                for key, value in indices_dict.items():
                    ...
                    indices_temp[key][chunk_index] = value  # Use chunk_index instead of index
            del indices_dict
            gc.collect()
...
def calculate_acoustic_indices(
    ...
    Notes
    -----
    - The method first divides the DataFrame into smaller chunks, each of which is processed separately 
      to calculate the acoustic indices. The results are saved as temporary CSV files.
    ...
    >>> result_df = calculate_acoustic_indices(df, 'file_path', indices_methods, pre_calc, parallel=True)
    """
    ...
    df_chunks = [df_init.iloc[i:i + chunk_size] for i in range(0, len(df_init), chunk_size)]
